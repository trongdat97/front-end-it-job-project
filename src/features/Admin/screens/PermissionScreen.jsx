...
function createData(id, action, module) {
  return { id, action, module }
}

function PermissionScreen(props) {
  const classes = useStyles()
  const { permissionList, status } = useSelector(adminSelector)
  ...
  useEffect(() => {
    if (status === 'getAllPermission.fulfilled') {
      let temp = []
      permissionList.map((item) => {
        temp.push(createData(item?.id, item?.action, item?.module))
      })
      setRows(temp)
    }
  }, [status])
