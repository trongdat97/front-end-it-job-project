...
function createData(index, name, email, type, deletedAt, action) {
  return { index, name, email, type, deletedAt, action }
}

function DeleteUser(props) {
  const classes = useStyles()
  const { deletedJob, status, errorMessage } = useSelector(adminSelector)
  ...
  useEffect(() => {
    dispatch(getDeletedJob())
  }, [])

  useEffect(() => {
    if (status === 'restoreJob.fulfilled') {
      dispatch(getDeletedJob())
      dispatch(
        showSnackbar({
          type: SNACK_BAR_TYPE.SUCCESS,
          message: message.RESTORE_JOB_SUCCESS,
        })
      )
      dispatch(clearState())
    }
  ...
  useEffect(() => {
    if (status === 'getDeletedJob.fulfilled') {
      let temp = []
      deletedJob.map((item) => {
        temp.push(
          createData(
            ...
            <Button
              ...
              onClick={() => {
                dispatch(
                  showDialog({
                    onSubmit: () => {
                      dispatch(restoreJob(item?.id))
                    },
                    message: message.CONFIRM_RESTORE_JOB,
                  })
                )
              }}
            >
